...
export default function PrivacyPage() {
  return <>
    ...
    <p className="information-intro">This policy explains the information used to process orders and provide our company formation services.</p>
    <h2>Who handles your information</h2><BusinessDetails />
    <p>Just My LLC handles information submitted through this website for processing orders and delivering services. Our support address is the contact point for privacy questions and requests.</p>
    <h2>Information we collect</h2>
    <ul>
      <li>Orders: name, email, optional phone number, proposed company name and suffix, entity type, state, LLC membership category, S Corp eligibility declaration and preferred language.</li>
      <li>Service records: order references, itemized fees, order and payment status, policy acceptance timestamps, correspondence and documents you authorize us to handle.</li>
    ...
    <p>Our order form does not ask for identity documents, SSNs, bank login credentials or card details. Payment details are entered on the payment provider’s hosted checkout. We receive transaction identifiers and payment status, not full card numbers. If a later service step requires additional personal or ownership information, we will explain the purpose, required recipients and appropriate submission method before requesting it.</p>
    <h2>Why we use information</h2>
    <p>We use information to respond to inquiries, process payments, check eligibility, carry out authorized services, provide support, prevent abuse and meet applicable recordkeeping or legal obligations. Where a legal basis is required, this may be taking steps at your request before a contract, performing a contract, complying with law, legitimate interests in secure operation and customer support, or consent where required. We do not use an order as consent to marketing.</p>
    <h2>Sharing and service providers</h2>
    <p>Order records are stored using our database infrastructure provider, Supabase. Payment, hosting, database and email providers process data needed to operate the service. Authorized formation and registered-agent providers, state agencies and the IRS may receive information required for a filing. Banks and payment platforms receive application information only as needed for the assistance you request and apply their own privacy policies.</p>
    ...
    <p>The website stores a private order-access token in session storage to let the same browser tab view the order and resume payment. Do not share this token. The website stores your language preference under <code>llc-lang</code> in browser local storage. You can change the language or clear site storage in your browser. The current application does not include advertising trackers or an analytics SDK. The site loads fonts from Google Fonts, which receives network information such as your IP address and browser headers when your browser requests them. See <a href="https://policies.google.com/privacy">Google’s Privacy Policy</a>.</p>
